/* 自动加载设备节点 */
/* 使用tasklet处理中断下部分，在tasklet_handler中操作led开启或关闭 */
...
static unsigned long state = 0; /* 记录led状态 */
...
static struct tasklet_struct my_tasklet; /* tasklet结构 */
...
static void tasklet_handler(unsigned long data)
{
	...
	file = filp_open("/dev/led0", O_RDWR, 0);

	file->f_op->write(file, &val, 1, &file->f_pos);
...
static int s3c24xx_buttons_open(struct inode *inode, struct file *file)
{
	...
	*gpgcon &= ~((0x3<<(3*2)) | (0x3<<(11*2)));
	...
	/* 注册tasklet */
	tasklet_init(&my_tasklet, tasklet_handler, state);
